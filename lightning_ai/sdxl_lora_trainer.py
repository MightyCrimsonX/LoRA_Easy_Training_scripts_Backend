...
def train(config: TrainingConfig) -> None:
    force_full_fp16 = config.mixed_precision == "full_fp16"
    if config.mixed_precision == "no" or force_full_fp16:
        accelerator_precision = None
    elif config.mixed_precision == "fp16":
        accelerator_precision = "fp16"
    elif config.mixed_precision == "bf16":
        accelerator_precision = "bf16"
    else:
        raise ValueError(f"Precisión mixta desconocida: {config.mixed_precision}")
    ...
    if force_full_fp16:
        LOGGER.info(
            "Modo full_fp16 activado: se deshabilita autocast y se fuerza float16 puro en el entrenamiento."
        )
    ...
    if config.mixed_precision in {"fp16", "full_fp16"}:
        ...
        for repeat in range(config.num_repeats):
            for step, batch in enumerate(dataloader):
                with accelerator.accumulate(unet):
                    ...
                    if force_full_fp16:
                        loss = F.mse_loss(model_pred, target.to(model_pred.dtype), reduction="mean")
                    else:
...
def parse_args() -> TrainingConfig:
    ...
    parser.add_argument(
        "--mixed-precision",
        choices=["no", "fp16", "full_fp16", "bf16"],
        default="fp16",
        help=(
            "Selecciona el modo de precisión. 'fp16' usa autocast/mixed precision, 'full_fp16' fuerza operaciones en fp16 "
