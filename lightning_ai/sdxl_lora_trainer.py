...
VAE_MODEL_CHOICES: Dict[str, str] = {
    "Stability AI SDXL VAE": "stabilityai/sdxl-vae",
    "Made by Ollin SDXL VAE FP16 Fix": "madebyollin/sdxl-vae-fp16-fix",
    "Turbo VAE XL": "segmind/turbo-sdxl-vae",
}
...
class TrainingConfig:
    ...
    def resolved_vae_model(self) -> Optional[str]:
        vae_key = (self.vae_model or "").strip()
        if vae_key:
            try:
                return VAE_MODEL_CHOICES[vae_key]
            except KeyError as exc:  # pragma: no cover - guard against inconsistent config
                raise ValueError(f"VAE desconocido: {vae_key}") from exc

        candidate = (self.vae_model_name_or_path or "").strip()
        if not candidate:
            return None
        if candidate in VAE_MODEL_CHOICES:
            return VAE_MODEL_CHOICES[candidate]
        return candidate
...
def train(config: TrainingConfig) -> None:
    ...
    base_assets = ensure_model_assets(
        resolved_model_name,
        "base",
        config.base_model or resolved_model_name,
    )
    ...
    resolved_vae_model = config.resolved_vae_model()
    if resolved_vae_model:
        LOGGER.info(
            "VAE seleccionado: %s",
            f"{config.vae_model} -> {resolved_vae_model}" if config.vae_model else resolved_vae_model,
        )
        vae_assets = ensure_model_assets(
            resolved_vae_model,
            "vae",
            config.vae_model or resolved_vae_model,
        )
...
def parse_args() -> TrainingConfig:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=sorted(VAE_MODEL_CHOICES),
